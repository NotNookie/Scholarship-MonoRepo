    ...
    <div className="hidden md:flex md:w-2/5 bg-primary-dark flex-col justify-between p-10 text-on-primary shrink-0">
      ...
      <div>
        <h2 className="text-2xl font-bold leading-snug mb-4">
          Managing scholarships for the youth of Sta. Cruz, Laguna.
        </h2>
        <p className="text-on-primary/70 text-sm leading-relaxed mb-8">
          Access the LYDO Management Portal to review applications, manage scholars,
          and coordinate scholarship activities.
        </p>
        <div className="flex items-start gap-3 bg-white/10 rounded-lg p-4">
          ...
          <div>
            ...
            <p className="text-xs text-on-primary/60 leading-relaxed">
              This portal is restricted to authorized LYDO personnel. All access attempts are logged.
            </p>
          </div>
        </div>
      </div>
...
export function AdminLoginPage() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: ({ session_token }) => {
      navigate('/admin/login/verify', { state: { session_token } })
    },
    onError: (err) => {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 bg-surface">
        <div className="w-full max-w-md">
          ...
          <h1 className="text-3xl font-bold text-content mb-2">Staff Access</h1>
          <p className="text-base text-content-muted mb-8">
            Enter your credentials to access the LYDO Management Portal.
          </p>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                <input
                  id="email"
                  type="email"
                  placeholder="staff@lgu-stacruz.gov.ph"
                  autoComplete="email"
          ...
          {import.meta.env.DEV && (
            <button
              type="button"
              onClick={() => {
                login({ id: 99, name: 'Admin User', role: 'super_admin', email: 'admin@test.com' }, 'dev-admin-token')
