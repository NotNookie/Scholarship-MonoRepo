    ...
    <div className="hidden md:flex md:w-2/5 bg-primary-dark flex-col justify-between p-10 text-on-primary shrink-0">
      ...
      <div>
        <h2 className="text-2xl font-bold leading-snug mb-4">
          One last step to secure staff access.
        </h2>
        <p className="text-on-primary/70 text-sm leading-relaxed mb-8">
          Two-factor authentication ensures only authorized LYDO personnel can access
...
export function AdminLoginVerifyPage() {
  ...
  useEffect(() => {
    if (!sessionToken) navigate('/admin/login', { replace: true })
  ...
  const verifyMutation = useMutation({
    mutationFn: (data) =>
      api
        .post('/auth/verify-otp', { session_token: sessionToken, otp: data.otp })
        .then((r) => r.data),
  ...
  const resendMutation = useMutation({
    ...
    onSuccess: () => toast.success('A new code has been sent to your registered contact.'),
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 bg-surface">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-content-muted mb-8 leading-relaxed">
            Enter the 6-digit verification code sent to your registered contact to complete sign-in.
          </p>
