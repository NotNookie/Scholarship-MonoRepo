...
import {
  CalendarDays, Clock, MapPin, Users, PencilLine, Trash2, Send, X, Loader2,
    ...
    <article className={`bg-surface border border-border border-l-4 ${isDraft ? 'border-l-tertiary' : 'border-l-secondary'} rounded-xl shadow-card overflow-hidden`}>
      <div className="p-5">
        <div className="flex items-center gap-2 flex-wrap mb-3">
          <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${type.bg} ${type.color}`}><Icon size={12} /> {type.label}</span>
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${isDraft ? 'bg-surface-alt text-content-muted border-border' : 'bg-tertiary-light text-tertiary-dark border-tertiary/30'}`}>{isDraft ? 'Draft' : 'Published'}</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-x-6 gap-y-1.5 mt-3 text-sm text-content-muted">
          <span className="inline-flex items-center gap-2"><CalendarDays size={14} className="text-content-disabled" />{formatDate(event.date)}</span>
          {(event.start_time || event.end_time) && (
            <span className="inline-flex items-center gap-2"><Clock size={14} className="text-content-disabled" />{[event.start_time, event.end_time].filter(Boolean).join(' – ')}</span>
          )}
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-x-6 gap-y-1.5 mt-1.5 text-sm text-content-muted">
          {event.location && <span className="inline-flex items-center gap-2"><MapPin size={14} className="text-content-disabled" /> {event.location}</span>}
          {event.target && <span className="inline-flex items-center gap-2"><Users size={14} className="text-content-disabled" /> Target: {event.target}</span>}
        </div>
      </div>
      <div className="px-5 py-3 bg-surface-alt border-t border-border flex items-center gap-3">
        {isDraft && (
          <button onClick={() => onPublish(event)} disabled={busy} className="inline-flex items-center gap-1.5 text-xs font-semibold bg-primary text-on-primary px-4 py-2 rounded-lg hover:bg-primary-dark transition-colors disabled:opacity-50">
            <Send size={13} /> Publish Schedule
          </button>
        )}
        <button onClick={() => onEdit(event)} className="inline-flex items-center gap-1.5 text-xs font-semibold text-content-muted border border-border px-4 py-2 rounded-lg hover:border-primary hover:text-primary transition-colors">
          <PencilLine size={13} /> Edit Details
        </button>
        <button onClick={() => onDelete(event)} className="ml-auto inline-flex items-center gap-1.5 text-xs font-medium text-content-muted hover:text-danger transition-colors" aria-label="Delete event"><Trash2 size={14} /></button>
      </div>
    </article>
  )
}
      ...
      <div className="relative bg-surface rounded-xl shadow-modal w-full max-w-sm p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-content-muted mt-1 leading-relaxed">"{event.title}" will be permanently removed.</p>
          </div>
        </div>
...
export function EventsPanel({ creating, onCloseCreate }) {
  ...
  const saveMutation = useMutation({
    mutationFn: (p) => (p.id ? api.put(`/admin/schedules/${p.id}`, p) : api.post('/admin/schedules', p)),
    onSuccess: () => { toast.success('Event saved.'); invalidate(); setEditing(null); onCloseCreate() },
    onError: (e) => toast.error(e?.response?.data?.message ?? 'Could not save event.'),
  })
  const publishMutation = useMutation({
    mutationFn: (e) => api.patch(`/admin/schedules/${e.id}`, { status: 'published' }),
  ...
  const deleteMutation = useMutation({
    mutationFn: (e) => api.delete(`/admin/schedules/${e.id}`),
    onSuccess: () => { toast.success('Event deleted.'); invalidate(); setDeleting(null) },
  ...
  const shown = useMemo(() => events.filter((e) => {
    ...
    return e.status !== 'draft' && (!d || d >= today)
  }), [events, tab, today])
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 flex flex-col gap-5">
        <div className="flex items-center gap-1 border-b border-border">
          {TABS.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)} className={`text-sm font-semibold px-4 py-2.5 -mb-px border-b-2 transition-colors ${tab === t.key ? 'border-primary text-primary' : 'border-transparent text-content-muted hover:text-content'}`}>
              {t.label}
        ...
        {isPending ? (
          ...
          <div className="space-y-4">
            {shown.map((e) => (
              <EventCard key={e.id} event={e} busy={busy} onEdit={setEditing} onDelete={setDeleting} onPublish={(ev) => publishMutation.mutate(ev)} />
            ))}
          </div>
        ) : (
      ...
      {showModal && (
        <EventModal
          ...
          onSubmit={(form) => saveMutation.mutate(editing ? { ...editing, ...form } : form)}
        />
      )}
      {deleting && (
        <DeleteModal event={deleting} isPending={deleteMutation.isPending} onClose={() => setDeleting(null)} onConfirm={() => deleteMutation.mutate(deleting)} />
      )}
    </div>
  )
}
