...
- [x] `[In app]` Live theme switching — presets **and custom colours** apply live across the whole UI and persist per device (`store/uiThemeStore.js` + `tenant/themePresets.js`, layered over the tenant palette by TenantProvider). Now on its **own Appearance page** (`/admin/maintenance/appearance`) with a **live preview gallery** (app header, buttons, status pills, form field, banner, stat cards, card) that reskins as you edit. **Custom theme:** Primary + Secondary via a native colour wheel + hex (`lib/color.js` auto-derives dark/light/text-on shades for contrast); an **Advanced** panel exposes the full palette + per-shade overrides *(= §H4)*
