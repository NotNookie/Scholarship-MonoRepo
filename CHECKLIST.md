...
- [x] `[In app]` **Settings area optimized** — unified under a persistent sub-nav + settings search/quick-jump (`SettingsLayout`, replaces hub-and-spoke round-trips). The Organization Profile mega-form gained a sticky in-page section nav (General · Contact · Public Content · Application · Theme + search deep-links), a **dirty-aware Save** ("Saved" when clean), a **sticky unsaved-changes bar** (Save/Discard), an **unsaved-changes guard** on navigate (`useBlocker`) + `beforeunload`, and a "theme applies live · other settings save on submit" clarity note.
- [x] `[In app]` **Decision-surface parity + undo everywhere** — Appeals & Renewals now match the Queue: **keyboard triage** (`j`/`k` move, `a`/`r`/`i` decide), **bulk actions**, and **Undo toasts** (shared `lib/undoToast`, revertable via mock `/revert`). Undo also covers municipal **user activate/deactivate** and **inline role change**. Mock adapter + demo data extended so appeals/renewals decisions, reverts, bulk, and user CRUD actually mutate the store (+ 5 seeded staff accounts).
- [x] `[In app]` **Dashboard "needs attention" band** — leads with pending review · scholars at-risk · renewals to review · open appeals, each a link to that filtered list (live counts).
- [x] `[In app]` **URL-synced filters** — Verification Queue (status + school year), Scholar Monitoring (status, tied to the cards) and Appeals round-trip filters through the URL (bookmarkable, back/forward works).
- [x] `[In app]` **Applicant Records bulk export** — row checkboxes + select-all; "Export selected (N)" to CSV.
- [x] `[In app]` **Admin command palette (⌘K)** — `components/admin/CommandPalette`: jump to any admin page or straight to an applicant/scholar by name; header "Search ⌘K" button + global shortcut.
...
- [x] `[In app]` Live theme switching — presets **and custom colours** apply live across the whole UI and persist per device (`store/uiThemeStore.js` + `tenant/themePresets.js`, layered over the tenant palette by TenantProvider). Now on its **own Appearance page** (`/admin/maintenance/appearance`) with **7 presets** (each with its real colour swatch) and a **live preview gallery framed as a browser mock** (dashed frame, window chrome, PREVIEW badge — app header, buttons, status pills, form field, banner, stat cards, card) that reskins as you edit. **Custom theme:** Primary + Secondary via a native colour wheel + hex (`lib/color.js` auto-derives dark/light/text-on shades for contrast); an **Advanced** panel exposes the full palette + per-shade overrides *(= §H4)*
